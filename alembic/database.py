...
from sqlalchemy import Integer, ForeignKey, String, Column, MetaData, create_engine,Table
...
eng = create_engine('mysql+pymysql://root:atom_pass@192.168.0.107:3336', pool_recycle=3600)
...
# domain = Table('hh_domain', metadata,
#             Column('id', Integer, primary_key=True),
#             Column('name', String(50)))
# mapper(HhDomain, domain)
...
def get_cianuser(**data):  # всё по id
    if data:
        q = Cian.query()
        for attr, value in data.items():
...
# удаление ################################################################################################
def del_usercian(id):
   #CianUser.delete().where(CianUser.c.id =id) --здесь нужен mapping но мы не знаем как это сделать, т.к. ошибка
# вызывается
    pass
# обновления ################################################################################################
...
def add_cian(id_cian, count_rooms, station, price, floor, square, pr_square, adress):
...
def add_cian_user( count_rooms, station, price, floor, square, pr_square, adress):
    station_id = get_id_station(station)
